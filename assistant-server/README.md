...
MCP 服务端用的是 [homeassistant-ai/ha-mcp](https://github.com/homeassistant-ai/ha-mcp)，两种装法：

**方式一：HA 加载项（推荐，适用于 HA OS / Supervised）**

1. HA 里进 **设置 → 加载项 → 加载项商店 → 右上角 ⋮ → 仓库**，
   添加 `https://github.com/homeassistant-ai/ha-mcp`；
2. 在商店里找到 **ha-mcp** 安装并启动（无需配置 token，加载项自动拿到 HA 权限）；
3. 打开加载项**日志**，里面会打印 MCP 的访问地址，把它填进本目录 `.env` 的
   `HA_MCP_URL`。

**方式二：Docker（HA Container 等没有加载项商店的场景）**

仓库里带了 docker-compose 文件，需要两个环境变量：

```bash
HOMEASSISTANT_URL=http://<HA地址>:8123
HOMEASSISTANT_TOKEN=<HA 长期访问令牌>   # HA 个人资料页 → 安全 → 长期访问令牌
```

启动后同样把它暴露的 Streamable HTTP 地址填进 `HA_MCP_URL`。

装好后重启本 server，启动日志出现 `[ha] 已连接 Home Assistant MCP` 即接通。
