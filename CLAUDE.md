...
- **小米音箱三条实测规律（改音箱推送逻辑前必读）**：playing 时 play_media 可靠；
  被我们 media_pause 停在推送媒体位上时 play_media 被无视；media_play 恢复旧内容。
  所以 `pushToSpeaker` = 需要时先 media_play 唤醒（暂停点在 6s 静音垫内，无声）→500ms→
  play_media 替换；播完按精确时长+3s 定时 media_pause 防 repeat=all 循环。
